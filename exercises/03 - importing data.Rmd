...
Let's load the packages we will need. Packages contain useful code that is written by other people for our use. We will install (install.packages()) and load (library()) the 'tidyverse' package, which loads all packages in the 'tidyverse' (see: https://www.tidyverse.org/packages/). These are very useful for data wrangling and visualization. 
