...
Let's work with our portal data and dplyr. We will examine, manipulate, summarize, and plot the data.

dpylr (https://dplyr.tidyverse.org/), in my opinion, is one of the most useful packages in R. Check out the website and read about it. These R documentation websites usually have very helpful tutorials (vignettes). There is also a helpful blog post by a Sean Anderson (a former lab-mate of mine, https://seananderson.ca/2014/09/13/dplyr-intro/).

dplyr is in the tidyverse, so we don't need to load it separately.

We've already used some dplyr functions! We have used glimpse(), mutate(), and the %>% (pipe). Let's build on these. 
...
species <- select(dat, -species) # this will remove the column from the dataframe
...
Now let's unleash the power of group_by() and summarize()! I use group_by() all of the time. It is a really useful function, and not just with summarize.
...
Challenge: What if I wanted to only see the first five rows and arrange by 'record_id' ?
```{r}
...
Before we plot some data, let's look at some other functions that I find useful and use alot. 

First up, rename(). Rename() allows you to rename column headings. 
```{r}
