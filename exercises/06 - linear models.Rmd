...
Before we get started, let's check out our data. We have wing length, beak height, and beak length. We also have 11 different species, and males and females for all species. For this lesson, we won't worry about the different species, as we will get into that when we look at mixed effects models. 
...
Let's work with wing height and berak length and see if there is a relationship between them (question: is beak height correlated with wing length?)
...
Let's start with the fit line. We want to obtain the fitted value of y for each value of x. It is somewhat confusing that the function is called 'predict' and not 'fitted'. For other types of models, it is called fitted (you will see this later). This is one thing about R -- packages are developed by different people, so not only are there many ways to do things, but the same action can be called something different depending on what package you are using. Before obtaining the fitted values, the 95% CI, and the 95% PI, you have to set up a new 'dataframe'. For 95% CI, this will just be your x (whatever you used in your lm(y~x)) and for 95% PI, you will want to create a new object that is the same length as your data and has the same range. 

```{r}

newdat_fit <- expand.grid(
  log_wingl = morph$log_wingl)

# for the 'newdat', the predictors have to be named the EXACT same as in your model

fitted_beakh <- predict(fit2, newdata = newdat_fit) 

```

Because I usually want the fitted values and the CIs, I instead give this function more information. 


```{r}


fitted_dat_lm <- predict(fit2, newdata = newdat_fit,
                            interval = "confidence")

# Q: how do these differ?
```

Now, let's compute the predictions. First, we need to create a new 'dataframe' of new x values to predict from. These are different than your data. Right now, we are only fitting a model with one predictor but if you have more than one, this dataframe would have more columns. 
...
newdat_pred <- expand.grid(
  log_wingl = seq(from = min_dat, 
  						to = max_dat, 
  						length.out = length_dat))

pred_dat_lm <- predict(fit2, newdata = newdat_pred,
                            interval = "prediction")

```


We now want to create some plots. The easiest way to do this is to keep these dataframes separate, but you don't have to. You could add the fitted and predicted values back into morph. We are going to keep them separate since that is what I usually do. ggplot() can pull data from many different dataframes! 

But first, we have to merge our 'newdat' and our respective fitted_dat_lm and pred_dat_lm. To do so, we first need to turn these into dataframes. (Q: What are they?). We then should rename the columns so we know what is what. So let's use a little dplyr magic. Hint: use 'as_tibble()' to coerce them into dataframes and then use rename().
...
fitted_dat_lm <- fitted_dat_lm %>%
	as_tibble() %>%
	rename(fit_upr = upr,
				 fit_lwr = lwr) 

pred_dat_lm <- pred_dat_lm %>%
	as_tibble() %>%
	rename(pred_upr = upr,
				 pred_lwr = lwr) 
...
Now let's join these dataframes into their respective newdat objects we made earlier. We need to do this because when we plot the fit line and both the confidence and prediction intervals, we will want to use the log_wingl data, which is in newdat. There are many ways to join dataframes together. Enter '? merge' in the console. This is one of my favorties, although dplyr improved on this with the [join family](https://dplyr.tidyverse.org/reference/mutate-joins.html). For our purposes, we can use 'bind_cols' because we are joining dataframes that don't share any data. 

```{r}

fits <- fitted_dat_lm %>%
	bind_cols(newdat_fit)

preds <- pred_dat_lm %>%
	bind_cols(newdat_pred)

```


Now we are ready to make some plots! We will use ggplot() with geom_point() for the data, geom_line() for the fit line, and geom_ribbon for the confidence and prediction intervals. Plots with ggplot(), especially once you format them for publication, can get really long (lots of lines!). This is because ggplot() builds plots iteratively. This means we have to pay attention to the order in which we add parts to the plot. Let's see how this works and how the plot will change if we don't think about order. 

Another thing to note: because we are pulling from 3 different dataframes, we don't want to specify a dataframe in ggplot() [this would apply it to all subsequent parts of the plot]. Instead, we will want to specify the data for each geom. 

```{r}

ggplot() +  
	geom_point(data = morph, aes(x = log_wingl, y = log_beakh)) +
	geom_line(data = fits, aes(x = log_wingl, y = fit)) +
	geom_ribbon(data = fits, aes(ymin = fit_lwr, ymax = fit_upr, x = log_wingl)) +
	geom_ribbon(data = preds, aes(ymin = pred_lwr, ymax = pred_upr, x = log_wingl))
...
ggplot() +
		...
		geom_ribbon(data = fits, aes(ymin = fit_lwr, ymax = fit_upr, x = log_wingl),
																color = "grey", alpha = 0.2) +
		geom_line(data = fits, aes(x = log_wingl, y = fit), size = 0.5) 
...
Let's tackle the first question. Let's pretend our dataset already had data on males and females. I will add this in for our purposes. 
...
Let's make a table with all of these values and export them to a .csv.
...
fits_females_int <- predict(fit4, newdata = females_newdat_fits, # calling this object something else
                            interval = "confidence") %>%
	bind_cols(females_newdat_fits)
...
Clearly, beak height is not well predicted my mass. That's ok! Now, let's plot wing height on the x-axis.
